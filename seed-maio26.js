// ============================================================
// SEED MAIO/26 — script temporário
// Executa UMA VEZ ao clicar no botão. Cria:
// - 3 categorias novas (Lazer, Banco/Juros, Impostos)
// - 15 recorrências + lançamentos de maio das recorrências
// - 8 grupos de parcelas (37 lançamentos)
// - 60 lançamentos avulsos
// - Marca cofre IPVA como pago
// ============================================================
import {
  getFirestore, doc, setDoc, collection, addDoc, writeBatch, getDoc, getDocs, query, where, updateDoc
} from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";

export async function executaSeedMaio(db, toast){
  // ========== checa se já foi executado (proteção contra duplicação) ==========
  const flagRef = doc(db, 'config', 'seedMaio26Executado');
  const flagSnap = await getDoc(flagRef);
  if(flagSnap.exists()){
    if(!confirm('Seed de maio/26 já foi executado uma vez. Executar de novo VAI DUPLICAR todos os lançamentos. Tem certeza?')){
      toast('Cancelado.');
      return;
    }
  }
  ...
  const novasCategorias = [
    { id: 'lazer',      nome: 'Lazer',         teto: 0, icone: '🎬', ordem: 15, criadoEm: Date.now() },
    { id: 'bancojuros', nome: 'Banco/Juros',   teto: 0, icone: '🏦', ordem: 16, criadoEm: Date.now() },
    { id: 'impostos',   nome: 'Impostos',      teto: 0, icone: '📋', ordem: 17, criadoEm: Date.now() },
  ];
  const batch1 = writeBatch(db);
  for(const c of novasCategorias){
    batch1.set(doc(db, 'categorias', c.id), c);
  }
  await batch1.commit();

  // ========== 2. RECORRÊNCIAS (e lançamentos de maio das recorrências) ==========
  const recorrencias = [
    {desc:'Aluguel',             valor:1850.98, dia:5,  cat:'moradia',     cartao:''},
    {desc:'Energisa',            valor:246.23,  dia:18, cat:'moradia',     cartao:''},
    {desc:'ALSOL Energias',      valor:374.69,  dia:21, cat:'moradia',     cartao:''},
    {desc:'Digital Net',         valor:126.16,  dia:21, cat:'comunicacao', cartao:''},
    {desc:'TIM (Filipe)',        valor:83.99,   dia:7,  cat:'comunicacao', cartao:''},
    {desc:'TIM (Mari)',          valor:98.99,   dia:15, cat:'comunicacao', cartao:'2913'},
    {desc:'Unimed',              valor:1701.71, dia:11, cat:'saude',       cartao:''},
    {desc:'Porto Seguro',        valor:60.37,   dia:11, cat:'seguros',     cartao:''},
    {desc:'Minas Brasil',        valor:514.88,  dia:27, cat:'seguros',     cartao:''},
    {desc:'Netflix',             valor:59.90,   dia:23, cat:'assinaturas', cartao:'2913'},
    {desc:'Spotify',             valor:31.90,   dia:14, cat:'assinaturas', cartao:'2913'},
    {desc:'Amazon Prime BR',     valor:19.90,   dia:11, cat:'assinaturas', cartao:'2913'},
    {desc:'Amazon Prime Canais', valor:34.90,   dia:21, cat:'assinaturas', cartao:'2913'},
    {desc:'Apple (iCloud)',      valor:9.99,    dia:11, cat:'assinaturas', cartao:'3877'},
    {desc:'Claude AI',           valor:116.02,  dia:27, cat:'assinaturas', cartao:'2913'},
  ];

  // cria recorrências e o lançamento de maio de cada uma
  ...
  for(const r of recorrencias){
    ...
    batch2.set(recRef, {
      ...
      dataInicio: new Date(2026, 4, 1).toISOString(),
      criadoEm: Date.now(),
    });
    // criar lançamento de maio (já materializado, não vai duplicar pelo materializaRecorrenciasDoMes porque já existe)
    const ts = new Date(2026, 4, r.dia, 12, 0).getTime();
  ...
  // ========== 3. PARCELAS ==========
  const parcelas = [
    {desc:'Drogasil 2710',         valor:93.20,  parcelas:2, mes_inicio:5, cat:'saude',       cartao:'3877'},
    {desc:'Shein by King',         valor:118.98, parcelas:3, mes_inicio:5, cat:'pessoalmari', cartao:'3877'},
    {desc:'Lojas Brasileirinho',   valor:73.45,  parcelas:2, mes_inicio:5, cat:'filhos',      cartao:'3877'},
    {desc:'Mercado*Carrosomshop',  valor:51.32,  parcelas:7, mes_inicio:5, cat:'transporte',  cartao:'2913'},
    {desc:'Shopee*Ironkids',       valor:74.40,  parcelas:5, mes_inicio:5, cat:'filhos',      cartao:'3877'},
    {desc:'Shopee*tricomais',      valor:72.33,  parcelas:4, mes_inicio:5, cat:'pessoalmari', cartao:'3877'},
    {desc:'Pneustore CPX',         valor:102.50, parcelas:8, mes_inicio:5, cat:'transporte',  cartao:'2913'},
    {desc:'Duda Baby',             valor:77.20,  parcelas:6, mes_inicio:5, cat:'filhos',      cartao:'3877'},
  ];
  const batch3 = writeBatch(db);
  for(const p of parcelas){
    const grupoId = 'p' + Date.now() + Math.random().toString(36).slice(2,6);
    for(let i = 0; i < p.parcelas; i++){
      const data = new Date(2026, 4 + i, 15, 12, 0); // dia 15 de cada mês a partir de maio
  ...
  // ========== 4. AVULSOS ==========
  // formato: [dia, mes, descricao, valor, categoria, cartao]
  const avulsos = [
    // Alimentação
    ...
    // Assinaturas avulsas (Apple 2x, Canva)
  ...
  // dividir em batches de 400 (limite Firestore = 500)
  ...
  try {
    ...
    console.warn('Não consegui atualizar cofre IPVA:', e);
  }

  // ========== 6. MARCAR SEED EXECUTADO ==========
  await setDoc(flagRef, {
    executadoEm: Date.now(),
    totalLancamentos: avulsos.length + recorrencias.length + 37,
  });

  toast('✓ Seed maio/26 importado! Aguarde o app sincronizar.');
}
